...
**成长与进阶（火纹核心三件套）**：
- **转职**：单位 Lv10+ 且持有转职证（每章通关获 1 枚），行动菜单「转职」进阶为高级职——
  领主→大将、重骑→圣骑士、弓兵→神射手、魔道→贤者、剑士→剑圣、修女→主教、天马→天马将军、重甲→将军。
  属性大幅跃升、成长率提高、解锁职业技（贤者/主教转职后可攻可治）。
- **职业技**：剑圣必杀+20、神射手必杀+15、贤者魔力+、主教祈祷治疗+、将军大盾几率半伤、
  天马将军回避+15、圣骑士坚韧、大将鼓舞光环。战斗预测实时反映。
- **羁绊／支援**：相邻友军提供命中/回避/必杀加成，剧情 CP（罗伊×莉莉娜、罗伊×丽贝卡、
  菲尔×西娅、艾莉丝×加斯…）加成更高，鼓励抱团走位（头顶金「羁」标记）。
...
```bash
.venv/bin/python -m pytest tests/ -v     # 88 个纯逻辑单元测试
...
平衡基准（贪心 bot 模型含转职/羁绊、不重试，20 局）逐章胜率约 80/94/100/100/100/80/92/100/100/91，
全程通关约 50%。难点集中在前期（第1章、第6章坚守战）；真人有战术与败北重试，实际更宽松。改数值后请重跑模拟。

代码结构：`settings/unit/combat/grid/ai/save/story/records/paths/music/supports` 为零 pygame 依赖的纯逻辑层
